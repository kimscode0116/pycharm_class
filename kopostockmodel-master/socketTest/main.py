    ...
    def do_POST(self):
        if None != re.search('/api/v1/addrecord/*', self.path):
            ctype, pdict = cgi.parse_header(
                self.headers['content-type'])  # application/json;encoding=utf-8;lang=ko;loc=seoul;...
            ...
            if ctype == 'application/json':
                ...
                # print(tempDict)
                # print(tempDict.key())
                temp = tempDict['성명']+"님의 주소는 " + tempDict['주소']+" 이메일은 " + tempDict['email']
                print(tempDict.keys())
                self.send_response(200)
                self.send_header('Content-type', 'application/json')
                self.end_headers()
