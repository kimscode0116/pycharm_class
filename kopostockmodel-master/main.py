...
from matplotlib import font_manager, rc ## matplotlib 라이브러리로 저장 가능 ## 해당 라이브러리 변경사항 보기
...
import plotly.express as px #  conda install -c plotly plotly-orca //이거 추천하ㅈㅣ 않는 방법임 ( 그냥 matplotlib를 써도 됨)
import os
...
def getDiffMAs(code, df,n1,n2): ## 차이를 구해주는 것 (golden closs인지 dead closs인
    if n1 >= n2:
...
kospi_stocks = qc.download_stock_codes('kospi') ## 코스피 정보를 리스트 형태로 받아
